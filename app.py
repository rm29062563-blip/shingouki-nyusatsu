...
def fetch_kkj(params: dict) -> ET.Element:
    resp = requests.get(KKJ_API_URL, params=params, timeout=15)
    resp.encoding = "utf-8"
...
def api_search():
    ...
    params = {"Query": SIGNAL_QUERY, "Organization_Name": "国家公安委員会", "Count": count}
    if prefecture:
    ...
    try:
        ...
        return jsonify({"error": "官公需情報ポータルサイトへの接続に失敗しました。時間をおいて再度お試しください。", "results": []}), 502
    except ET.ParseError:
        return jsonify({"error": "検索結果の解析に失敗しました。", "results": []}), 502

    error_el = root.find("Error")
    if error_el is not None:
        return jsonify({"error": _text(error_el) or "検索でエラーが発生しました。", "results": []}), 400
    ...
    return jsonify({"error": None, "searchHits": search_hits, "results": results})
...
<div class="topbar">
  ...
  <div>
    <h1>信号機入札ナビ</h1>
    <p>信号機・信号灯器・信号制御機など信号機関連の入札公告のうち、国家公安委員会の案件だけを検索</p>
  </div>
</div>
...
async function runSearch(){
  ...
  try{
    ...
    currentPage = 1;
    statusEl.innerHTML = `ヒット件数: <b>${data.searchHits}</b> 件（取得: ${allResults.length}件）`;
    renderPage();
  }catch(e){
...
def index():
    import json
    html = INDEX_HTML.replace("__PREF_JSON__", json.dumps(PREFECTURES, ensure_ascii=False))
